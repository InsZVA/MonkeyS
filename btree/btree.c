...
void BTree_split_child(BTNode* parent, int index, BTNode* node)
{
    ...
    // 拷贝 node 后半部分关键字及记录指针,然后将node后半部分置为0。
    for (i = 0; i < newNode->keynum; ++i){
        newNode->key[i] = node->key[BTree_D + i];
        newNode->pRecord[i] = node->pRecord[BTree_D + i];
        node->key[BTree_D + i] = 0;
        node->pRecord[BTree_D + i] = 0;
    }
    ...
    for (i = parent->keynum - 1; i >= index; --i) {
        ...
        parent->pRecord[i + 1] = parent->pRecord[i];
    }

    parent->key[index] = node->key[BTree_D - 1];
    parent->pRecord[index] = node->pRecord[index];
    ++parent->keynum;

	node->key[BTree_D - 1] = 0;
    node->pRecord[BTree_D - 1] = 0;
...
void BTree_insert_nonfull(BTNode* node, KeyType key,const void* pRecord)
{
    assert(node);
    ...
    if (node->isLeaf) {
        ...
        while (i >= 0 && key < node->key[i]) {
            ...
            node->pRecord[i + 1] = node->pRecord[i];
            --i;
        }

        node->key[i + 1] = key;
        node->pRecord[i + 1] = pRecord;
        ++node->keynum;
    ...
    // 节点是内部节点
        ...
        BTree_insert_nonfull(node->child[i], key, pRecord);
    }
}

void BTree_insert(BTree* tree, KeyType key,const void* pRecord)
{
#ifndef RELEASE_VERSION
	printf("BTree_insert:\n");
#endif
    ...
    if (root->keynum == (ORDER-1)) {
        ...
        *tree = node;
        ...
        BTree_insert_nonfull(node, key, pRecord);
    }

    // 根节点未满，在当前节点中插入 key
    else {
        BTree_insert_nonfull(root, key, pRecord);
    }
}
...
void BTree_merge_child(BTree* tree, BTNode* node, int index)
{
    ...
    const void* pRecord = node->pRecord[index];
    ...
    for (i = index; i < node->keynum - 1; ++i) {
        ...
        node->pRecord[i] = node->pRecord[i + 1];
        node->child[i + 1] = node->child[i + 2];
    }
    node->key[node->keynum - 1] = 0;
    node->pRecord[node->keynum - 1] = 0;
...
void BTree_recursive_remove(BTree* tree, KeyType key)
{
    ...
    if (!root) {
        printf("Failed to remove %llu, it is not in the tree!\n", key);
        return;
    }
...
============================================================*/
    ...
    const void *leftpRecord, *rightpRecord;
    if (index < node->keynum && node->key[index] == key) {
        ...
        // 然后在左孩子节点中递归删除元素leftKey。
            ...
            leftpRecord = leftChild->pRecord[leftChild->keynum - 1];
            node->key[index] = leftKey;
            node->pRecord[index] = leftpRecord;
        ...
        // 然后在右孩子节点中递归删除元素rightKey。
            ...
            rightpRecord = rightChild->pRecord[0];
            node->key[index] = rightKey;
            node->pRecord[index] = rightpRecord;
	...
	/*二、结点中未找到了关键字key的情况.*/
		...
		//相邻右兄弟相对富有，则该孩子先向父节点借一个元素，右兄弟中的第一个元素上移至父节点所借位置，并进行相应调整。
                    ...
                    for (j = child->keynum; j > 0; --j) {//元素后移
                        ...
                        child->pRecord[j] = child->pRecord[j - 1];
                    ...
                    child->pRecord[0] = node->pRecord[index - 1];
                    ++child->keynum;

                    node->key[index - 1] = leftSibling->key[leftSibling->keynum - 1];
                    node->pRecord[index - 1] = leftSibling->pRecord[leftSibling->keynum - 1];

					leftSibling->key[leftSibling->keynum - 1] = 0;
                    leftSibling->pRecord[leftSibling->keynum - 1] = 0;
...
{
    bool suc = false;
    *pos = binarySearch(tree, 0, tree->keynum - 1, key, &suc);
    if(suc) return tree;
    /*
    ...
    */
    ...
    return BTree_recursive_search(tree->child[*pos], key, pos);
}
...
//===============================create===============================
void BTree_create(BTree* tree, const KeyType* data, const void** recordes, int length)
{
    assert(tree);
    ...
    for (i = 0; i < length; i++) {
		...
		if (pos!=-1)
		{
			printf("this key %llu is in the B-tree,not to insert.\n",data[i]);
		}else{
			BTree_insert(tree, data[i], recordes[i]);//插入元素到BTree中。
		}
